...
public class InputListener {
  ...
  public void run() {
    //==============================================================================================
    onKey(Controls.UP, "Up",
        () -> {
          if (MCClient.getClientEntity() != null) {
            synchronized (MCClient.getClientEntity()) {
              Entity player = MCClient.getClientEntity();
              if (player==null) return;
              player.getComponent(PlayerControlComponent.class).up();
            }
          }
        });

    onKey(Controls.LEFT, "Left",
        () -> {
    ...
    onKey(Controls.DOWN, "Down",
        () -> {
          if (MCClient.getClientEntity() != null) {
            synchronized (MCClient.getClientEntity()) {
              Entity player = MCClient.getClientEntity();
              if (player==null) return;
              player.getComponent(PlayerControlComponent.class).down();
            }
          }
        });

    onKey(Controls.RIGHT, "Right",
        () -> {
          if (MCClient.getClientEntity() != null) {
            synchronized (MCClient.getClientEntity()) {
              Entity player = MCClient.getClientEntity();
...

    {
      ...
      MCClient.getClientState().getIdHashTable().forEach((k, v) -> {
        System.out.println(
            "[" + k + "-" + v.getComponent(DataComponent.class).getId() + "] ["
                + v.getComponent(
    ...
    getInput().
        addAction(new UserAction("Kill") {
          @Override
          protected void onActionBegin() {
            doAction(new Pair<>(MessageType.HP,
                new HPAction(MCClient.getClientId(), MCClient.getClientId(), 100000)));
          }
        }, KeyCode.K, VirtualButton.B);

    getInput().

        addAction(new UserAction("Use") {
          @Override
          protected void onActionBegin() {
            if (MCClient.getClientEntity() != null) {
              synchronized (MCClient.getClientEntity()) {
                ...
                getGameWorld().getEntitiesByType(EntityType.BUTTON)
                    .stream()
                    .filter(
                        btn -> btn.hasComponent(CollidableComponent.class)
    ...
    getInput().
        ...
        addAction(new UserAction("Viewport") {
          @Override
          protected void onActionBegin() {
    ...
    getInput().
        ...
        addAction(new UserAction("Take/Throw") {
          @Override
          protected void onActionBegin() {
            if (MCClient.getClientEntity() != null) {
              synchronized (MCClient.getClientEntity()) {
                Entity player = MCClient.getClientEntity();
                if (player==null) return;
                List<Entity> list = player.getComponent(WeaponInventoryComponent.class)
                    .findWeapon();
                if (list.size() >= 1) {
                  ...
                  if (player.getComponent(WeaponInventoryComponent.class).getActiveWeapon()
                      != null) {
                    SoundsController.getSoundsController().play(Sounds.pickup_weapon);
                    doAction(new Pair<>(MessageType.WEAPON,
                        new WeaponAction(WeaponActionType.THROW,
                            player.getComponent(DataComponent.class).getId(),
                            player.getComponent(WeaponInventoryComponent.class)
                                .getActiveWeapon()
