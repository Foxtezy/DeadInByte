...
import com.almasb.fxgl.logging.Logger.Companion.get
 ...
 */
class TilesetLoader(private val map: TiledMap, private val path: String) {

    private val log = get<TilesetLoader>()
...
//                    srcy)
    ...
    private fun loadImage(tilesetImageName: String, transparentcolor: String, w: Int, h: Int): Image {
        ...
        val image = try {
            val ext = path.substringBeforeLast("/") + "/"

            val stream = FileInputStream(ext + imageName)
            ...
            log.warning("$imageName cannot be loaded using mapURL=$path", e)
    ...
    fun copy(): TilesetLoader = TilesetLoader(map, path)
}